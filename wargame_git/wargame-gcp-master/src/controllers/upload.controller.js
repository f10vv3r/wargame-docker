...
async function runDocker(serverFilePath) {
    ...
    // 명시적으로 docker-compose 경로 사용
    const dockerComposeCmd = '/usr/bin/docker-compose';

    return new Promise((resolve, reject) => {
        const dockerCompose = spawn(dockerComposeCmd, ['-f', dockerComposePath, 'up', '--build'], {
            stdio: 'inherit', // 로그를 바로 터미널에 출력
        });
        resolve();
